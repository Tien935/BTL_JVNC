...
const MyAppointments = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const mappedData = data.map((apt) => ({
          ...
          specialty:
            apt.doctor && apt.doctor.specialty
  ...
  const handleCancel = async (id, status) => {
    if (status === "CONFIRMED") {
      if (
        window.confirm(
          `Bạn có chắc muốn gửi yêu cầu hủy lịch hẹn ${id} tới Admin không?`,
        )
      ) {
        try {
          const response = await fetch(
  ...
  const filteredAppointments = appointments.filter((apt) => {
    const matchesSearch =
      ...
      apt.id.toLowerCase().includes(filter.search.toLowerCase());
    const matchesStatus =
      filter.status === "Tất cả" || apt.status === filter.status;
    ...
    <div className="container mx-auto px-6 py-12 max-w-5xl">
              ...
              {loading ? (
                ...
                filteredAppointments.map((apt) => (
                  <tr key={apt.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="px-8 py-6">
                      <span
                        className={`px-4 py-1.5 rounded-full text-xs font-black uppercase tracking-wider ${apt.color}`}
                      >
                        {apt.status}
                      </span>
                    </td>
            ...
            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                ...
                <p className="text-slate-500 font-semibold text-sm mt-1">
                  Mã lịch hẹn: {selectedRecord?.appointment?.id}
                </p>
              </div>
              <button
            ...
            <div className="p-8 max-h-[70vh] overflow-y-auto">
              {loadingRecord ? (
                  ...
                  <div className="space-y-6">
                    <section>
                      <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center">
                        <i className="fas fa-stethoscope mr-2 text-teal-500"></i>{" "}
                        Triệu chứng & Bệnh sử
                      </h4>
                    ...
                    <section>
                      <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center">
                        <i className="fas fa-file-medical mr-2 text-teal-500"></i>{" "}
                        Chẩn đoán
                      </h4>
                    ...
                    <section>
                      <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center">
                        <i className="fas fa-pills mr-2 text-teal-500"></i> Đơn
                        thuốc & Điều trị
                      </h4>
                    ...
                    {selectedRecord.notes && (
                      <section>
                        <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-3 flex items-center">
                          <i className="fas fa-comment-medical mr-2 text-teal-500"></i>{" "}
                          Ghi chú & Dặn dò
                        </h4>
                ...
                <div className="py-20 text-center">
                  ...
                  <p className="text-slate-500 font-bold">
                    Chưa có kết quả chi tiết cho lịch hẹn này.
                  </p>
                </div>
              )}
            </div>
      ...
      <div className="mt-12 bg-teal-600 rounded-[2.5rem] p-10 text-white flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden shadow-2xl shadow-teal-100">
        ...
        <div className="relative z-10">
          ...
          <p className="text-teal-50 opacity-80 max-w-md">
            Tiếp tục chăm sóc sức khỏe của bạn bằng cách đặt một cuộc hẹn mới
            ngay hôm nay.
          </p>
        </div>
        <button
